...
<script>
	export default {
		data() {
			return {
				model: this.$store.state.invoices.find(f => f.id == this.$route.params.id) || { ref: null, subject: null, date: null, amount: null, notes: null }
			}
		},
		computed: {
		},
		...
		methods: {
			submit() {
				...
				this.$store.dispatch('UPDATE_INVOICE', { id: this.$route.params.id, data }).then(() => this.$router.push({ name: 'InvoiceList' }))
			}
		}
	}
</script>
