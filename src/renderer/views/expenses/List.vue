	...
	<div class="pb-2 mb-3">
		<nav class="navbar fixed-top flex-md-nowrap p-0 shadow w-100">
			<div class="col-md-2 mr-0 d-none d-sm-block p-1">
				<router-link :to="{name: 'CreateExpense'}" class="btn btn-block"><i class="fe fe-edit mr-2"></i>Nuevo</router-link>
			</div>
					...
					<tbody>
						<tr v-for="item in expenses">
							...
							<td class="py-1 align-middle text-right">
								<button type="button" class="btn btn-primary btn-sm" @click.prevent="$router.push({ name: 'ExpenseEdit', params: { id: item.id } })">Editar</button>
								<button type="button" class="btn btn-danger btn-sm" @click.prevent="$store.dispatch('DELETE_EXPENSE', item.id)">Eliminar</button>
							</td>
						</tr>
					</tbody>
				</table>
...
<script>
	export default {
		...
		computed: {
			expenses() {
				return this.$store.state.expenses.filter(p => this.search ? Object.values(p).some(v => typeof v == 'string' && v.toLowerCase().indexOf(this.search.toLowerCase()) >= 0) : 1)
			}
		},
		beforeCreate() { this.$store.dispatch('SELECT_EXPENSES') }
	}
</script>
